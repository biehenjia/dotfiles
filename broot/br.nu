# VENDORED — broot's own generated nushell launcher, checked in so a fresh clone
# has `br` without running `broot --install`. Do not hand-edit; regenerate with:
#   broot --print-shell-function nushell > ~/dotfiles/broot/br.nu
# Loaded via `use` from nushell/config.nu.
